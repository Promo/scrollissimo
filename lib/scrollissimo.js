...
(function(doc, win){
    ...
    Scrollissimo._catch = function(customScrollTop){
        var scrollTop = ( isNaN(customScrollTop) ? win.pageYOffset : customScrollTop), //calculate current scroll
    ...
    Scrollissimo._disableKnock = true;

    win.addEventListener('load', function(){
        docHeight = getDocHeight();
        windowHeight = win.innerHeight;

        // Prevent start scroll animating
        setTimeout(function(){
            var startScrollTop = win.pageYOffset,
                startProgress = startScrollTop / (docHeight - windowHeight);
            lastScroll = startScrollTop;
            Scrollissimo._render(startProgress, true);
            Scrollissimo._disableKnock = false;
        }, 100);
    });
