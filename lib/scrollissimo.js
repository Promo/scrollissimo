...
(function(_document, _window){
    ...
    var queues = [],
        ...
        //previous trigger page scroll progress 0..1
        lastProgress,

        //height of the document right after window loading
        startDocumentHeight,

        //height of the window right after window loading
        windowHeight,
    ...
    function toPercents(px, documentHeight){
        var scrollHeight = (documentHeight || startDocumentHeight || getDocHeight()) - _window.outerHeight;
        ...
        if(typeof px === 'string'){
            ...
            //if in pixels
                ...
                return (parseFloat(px) / scrollHeight);
            }
    ...
    Queue = function(timeline, start, maxSpeed){
        if(timeline){
            ...
            this.params = {
                start: toPercents(start || 0),
                duration: toPercents(timeline.duration() || 0)
            };
    ...
    Queue.Smoother.prototype.step = function(){
        ...
        if(Math.abs(delta) > this.maxSpeed){
            ...
            //jump by maximal speed value instaed of delta and render
     ...
     * @param progress {number} current scroll progress 0..1
     * @param enableSmoothing {boolean}
     * @private
     */
     function _render(progress, enableSmoothing){
        ...
        smoothQueues.forEach(function(queue){
            ...
            if(enableSmoothing){
                ...
                //run smoother
                queue._smoother.smooth(lastProgress, progress);
            }else{
    ...
    function _catch(customScrollTop){
        ...
        var scrollTop = (isNaN(customScrollTop) ? _window.pageYOffset : customScrollTop),
            ...
            progress = scrollTop / (startDocumentHeight - windowHeight);

        _render(progress, Scrollissimo.enableSmoothing);
    ...
    _window.addEventListener('load', function(){
        ...
        setTimeout(function(){
            ...
            var startScrollTop = _window.pageYOffset,
                ...
                //calculate start page scroll progress
                startProgress = startScrollTop / (startDocumentHeight - windowHeight);

            //remember start scroll position as a last scroll position
            lastScroll = startScrollTop;

            //render start progress
            _render(startProgress, false);
